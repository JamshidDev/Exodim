        ...
        <div class="grid">
          ...
          <div class="col-12 y-0 py-0">
            <span class="text-2xl font-semibold"
              >Lavozim yo'riqnomasi kiritilmagan xodimlar -
    ...
    methods:{
      get_List(params){
        this.controlLoaser(true)
          ViewService.get_ViewNotStuffCadry(params).then((res)=>{
            console.log(res.data.stafffiles.data);
              let number =
              (this.params.page - 1) * this.params.per_page;
              res.data.stafffiles.data.forEach((item) => {
              number++;
              item.number = number;
            });
              this.List = res.data.stafffiles.data;
              this.totalItem = res.data.stafffiles.pagination.total;
