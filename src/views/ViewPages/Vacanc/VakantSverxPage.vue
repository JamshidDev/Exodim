      ...
      <div class="grid">
        ...
        <div class="col-6 flex justify-content-end">
            <Button
                icon="pi pi-filter"
                label="Vakansiya"
                class=" p-button-success mx-2 p-button-sm"
              />
        </div>
        ...
        <Column style="min-width:80px; width:100px">
          ...
          <template #body="slotProps">
            <div
              ...
              "
            >
              <span>{{ slotProps.data.rate - slotProps.data.rate_sum }}</span>
            </div>
          </template>
        </Column>
  ...
  methods:{
    get_List(params){
      this.controlLoaser(true)
        ViewService.get_ViewVacanc(params).then((res)=>{
            console.log(res.data.vacancies);
            let number =
            (this.params.page - 1) * this.params.per_page;
            res.data.vacancies.data.forEach((item) => {
            number++;
            item.number = number;
          });
            this.List = res.data.vacancies.data;
            this.totalItem = res.data.vacancies.pagination.total;
