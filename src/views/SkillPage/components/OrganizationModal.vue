<template>
  <div class="grid px-3">
    <div class="col-12">
      ...
      >
        <Column field="apparat_id.name" header="apparat_id.name"> </Column>
        <Column field="name" header="">
          <td>
            <span class="w-full text-base font-medium ml-2">{{ slotProps.data.name }}</span>
          </td></Column
        >
        <Column header="">
        ...
        <template #groupheader="slotProps">
          <span class="text-base text-blue-600 font-medium text-center">{{
            slotProps.data.apparat_id.name}}</span> <span class="text-green-700 pl-4 font-medium">
                 <Chip
                :label="totalCounter(slotProps.data.apparat_id.name).toString()"
                class="mr-1 text-sm text-green-700 bg-green-100 font-bold"
              />
            </span>
        </template>
        <template #groupfooter="slotProps">
          <td colspan="1" style="text-align: right"> <span class="text-sm font-medium">Umumiy soni</span> </td>
          <td>  <Chip
                :label="totalCounter(slotProps.data.apparat_id.name).toString()"
                class="mr-1 text-sm text-green-700 bg-green-100 font-bold"
              /></td>
