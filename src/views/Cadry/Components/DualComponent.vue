<template>
  ...
  export default {
    ...
    methods: {
      ...
      changeProfession(){
        this.specialties_List =this.professions_List.filter((item) => item.id == this.professionsVal.id )[0].specialties;
        this.technicals_List = this.professions_List.filter((item) => item.id == this.professionsVal.id )[0].technicals;
      ...
      EditAndAdd(){
        ...
        if(!this.professionVal_Error && !this.specialtiesVal_Error && !this.technicalsVal_Error ){
            if(this.dialogType){
                this.dialog = false;
       
        DualService.create_Duals({cadry_id:this.$route.params.id, data}).then((res) => {
